...
void Database::CreateTableIfNotExists() const
{
    const auto createTableSQL = R"(
        ...
        CREATE INDEX IF NOT EXISTS idx_fingerprint ON tuning_records(parameter_fingerprint);
    )";
...
void Database::WriteToDatabase(const Record& record) const
{
    ...
    const char* insertSQL = "INSERT INTO tuning_records (parameter_fingerprint, kernel_source_path, best_result) VALUES (?, ?, ?)";
    sqlite3_stmt* stmt = nullptr;
    ...
    sqlite3_bind_text(stmt, 2, record.m_KernelSourcePath.string().c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, record.m_BestResult.dump().c_str(), -1, SQLITE_TRANSIENT);
